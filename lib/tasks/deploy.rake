...
namespace :deploy do
  desc "Deploy staging/production environments to Heroku."
  PRODUCTION_APP = 'good100k-production'
  STAGING_APP = 'good100k-staging'

  desc "Deploy to Heroku staging #{STAGING_APP} with migrations"
  task :staging_migrations => [:set_staging_app, :push, :off, :migrate, :restart, :on, :tag]
  
  task :staging_rollback => [:set_staging_app, :off, :push_previous, :restart, :on]

  desc "Deploy to Heroku production #{PRODUCTION_APP} with migrations"
